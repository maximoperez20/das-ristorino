-- Inserta católogos base: provincias, localidades y idiomas (idempotente)
SET NOCOUNT ON;
...
IF NOT EXISTS (SELECT 1 FROM localidades WHERE nom_localidad=N'Róo Cuarto' AND cod_provincia=@cod_cba)
  INSERT INTO localidades (nom_localidad, cod_provincia) VALUES (N'Róo Cuarto', @cod_cba);
...
IF NOT EXISTS (SELECT 1 FROM idiomas WHERE cod_idioma=N'es-AR')
  INSERT INTO idiomas (nom_idioma, cod_idioma) VALUES (N'Espaóol (Argentina)', N'es-AR');
...
IF NOT EXISTS (SELECT 1 FROM idiomas WHERE cod_idioma=N'pt-BR')
  INSERT INTO idiomas (nom_idioma, cod_idioma) VALUES (N'Portuguós (Brasil)', N'pt-BR');
